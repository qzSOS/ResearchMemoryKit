...
[Medical SLM](https://github.com/qzSOS/Medical-SLM) 是第一份可以直接检查的
真实项目案例。它展示了一个短周期科研原型如何使用：

- Current State 路由，以及数据、训练、评测和交付门控；
- 只追加的决策、失败路线、坑位和实验结论；
- 记录固定输入、命令、hash 与主张边界的轻量实验 registry；
- 独立的公开审查流程，移除机器标识、本地路径、权重、数据集和仅供发布者使用的说明。

仓库内置案例仍然是可执行的虚构示例。Medical SLM 作为独立公开仓库链接，
读者可以检查真实项目过程，而不需要把整个项目复制进 ResearchMemoryKit。
RMK 检查器验证的是结构和路由，不会验证其中医学结论或实验主张是否真实。
...
本项目由 [qzSOS](https://github.com/qzSOS) 创建。仓库内置示例均为虚构内容；
[Medical SLM](https://github.com/qzSOS/Medical-SLM) 是独立、可检查的真实公开
项目案例。私人项目如何准备公开材料单独写在
